<template>
      ...
      <div class="modal-detail">
            ...
            <div class="row-form d-flex">
              <div class="flex mr-10 block">
                ...
                <!-- <div class="d-flex text-field-form space-between">
                  ...
                  <div class="action">
                    <input class="input-file" type="file" ref="avatar" accept="image/*" @change="handleUploadAvatar"
                      tabindex="2">
                ...
                <div class="flex">
                  ...
                  <div class="d-flex justify-content mt-20">
                    ...
                    <BaseButton @click="cancelUploadAvatar" class="sub-button btn ml-10"
                      v-show="urlImage != Resource.PrefixImage + product.mainImage">
              ...
              <div class="flex mr-10 block">
                ...
                <div class="flex-column text-field-form space-between">
                  <div class="content">
                    <InputString :transmissionString="listImageTxt" :isReadonly="true" />
                  </div>
                  <div class="action">
                    <input class="input-file" multiple type="file" ref="imageDetail" accept="image/*"
                      @change="handleUploadImageDetail" tabindex="3">
                    <BaseButton @click="seeImageDetail" v-show="listUrlImage.length != 0" class="main-button btn ml-10">
                      {{ Resource.Button.See }}
                    </BaseButton>
                    <BaseButton @click="cancelUploadListImage" class="sub-button btn ml-10"
                      v-show="listUrlImage.length != 0">
                      {{ Resource.Button.Cancel }}
                    </BaseButton>
                  </div>
                </div>
              ...
              <div class="flex text-aria-form">
                ...
                <div class="flex-row border">
                  <textarea :placeholder="Resource.Placehoder.StandardPorts" class="aria-form flex" rows="4"
                    v-model.trim="product.standardPorts" tabindex="37"></textarea>
                </div>
              </div>
            </div>
        ...
        <div class="modal-footer">
          <div class="flex-row flex-end">
            ...
            <BaseButton tabindex="41" @keydown.tab.prevent.exact="focusProductName = true;" @mousedown="closeForm"
  ...
  <ImportProduct @refreshData="reloadData" @showToast="showToast" :productId="product.productID"
    :closeFormImport="closeFormImport" v-if="isShowFormImport" />
  ...
  <ViewImage :link="listUrlImage" v-show="isShowListImage" @focusoutImage="isShowListImage = false;" />
</template>
<script>
...
import ImportProduct from '../Modal/ImportProduct.vue';
...
export default {
  ...
  data() {
    return {
      ...
      isShowListImage: false, // Cờ điều khiển đóng mở xem ảnh sản phẩm

      isShowListGift: false, // Cờ điều khiển ẩn hiện xem quà tặng kèm
      ...
      productInit: { // thông tin sản phẩm mặc định của form thêm mới
        ...
        mainImage: Resource.DefaultMainImage, // avatar sản phẩm
        ...
        price: 0, // giá gốc
        discount: 0, // giảm giá
        status: 2, // trạng thái sản phẩm (0: tạm ngừng bán, 1: đang bán, 2: hàng sắp về)
      ...
      listImage: null, // Danh sách ảnh sản phẩm
  ...
  methods: {
    ...
    handleUploadAvatar() {
      ...
      this.urlImage = URL.createObjectURL(this.$refs.avatar.files[0]);
      this.mainImage = this.$refs.avatar.files[0];
    },
    ...
    */
    ...
    async sendRequestUpdate() {
      ...
      try {
        ...
        // };
        console.log(this.listGifts);
        await axios.put("Product/" + this.product.productID, {
          product: this.product, listImages: this.listImage,
          mainImage: this.mainImage, listGift: this.listGifts
        }, {
          headers: {
