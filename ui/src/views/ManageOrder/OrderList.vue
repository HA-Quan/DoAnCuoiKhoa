                ...
                <table cellpadding="0" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th class="col-180">
                                <div class="flex-row">
                                    <p class="title-table">{{ Resource.Label.OrderCode }}</p>
                                </div>
                            </th>

                            <th class="col-180">
                                <div class="flex-row pointer" @click="sortData(Enum.Sort.UsernameAsc)">
                                    <p class="title-table">{{ Resource.Label.OrderBy }}</p>
                                    <div class="icon-sort" :class="{
                                        'desc': this.sort == Enum.Sort.UsernameDesc,
                                        'asc': this.sort == Enum.Sort.UsernameAsc
                                    }"></div>
                                </div>
                            </th>
                            ...
                            <th class="col-100 pointer" @click="sortData(Enum.Sort.PriceAsc)">
                                <div class="flex-row">
                            ...
                            <th class="col-150 pointer" @click="sortData(Enum.Sort.TimeAsc)">
                                <div class="flex-row">
                                    <p class="title-table ">{{ Resource.Label.TimeOrder }}</p>
                                    <div class="icon-sort" :class="{
                            ...
                            <th class="col-120 pointer" @click="sortData(Enum.Sort.StatusAsc)">
                                <div class="flex-row">
                    ...
                    <tbody v-if="totalCount > 0">
                        <tr v-for="(order, index) in listOrder" @click.exact="focusIndex = index"
                            ...
                            <td>
                                {{ convertTime(order.createdDate) }}
                            </td>
                            <td >
                                <BaseButton @click.prevent="eventContextMenu(order)" class="btn button-blue">
                    ...
                    <ContextStatus :positionTop="positionTop" :positionLeft="positionLeft" v-if="isShowContextMenu"
                        @close="closeContextMenu" @updateStatus="updateStatus" :statusInit="orderNow.status" />
...
import FormOrderDetail from '../ManageOrder/FormOrderDetail.vue';
import ContextStatus from '@/components/base/ContextStatus.vue';
...
export default {
    name: "OrderList",
    components: {
        BaseLoading, BaseButton, BaseCombobox, BaseCheckbox, BaseToastMessage, BasePopup, BasePaging, FormOrderDetail, ContextStatus
    },
    data() {
        return {
            Resource: Resource,
            ...
            isShowContextMenu: false, //cờ điều khiển đóng mở form contextMenu
    ...
    methods: {
        ...
        /**
         * Sự kiện khi click vào trạng thái đơn hàng
         * Author: HAQuan HAQUAN(26/08/2023)
         */
        eventContextMenu(order) {
            event.preventDefault();
            this.isShowContextMenu = true;
            this.orderNow = order;
            this.positionTop = event.clientY > 580 ? event.clientY - 152 : event.clientY;
            this.positionLeft = event.clientX > 1384 ? event.clientX - 140 : event.clientX;

            console.log(event.clientY + "------" + event.clientX);
        },

        updateStatus(value) {
            this.requestUpdateStatus(this.orderNow.orderID, value);
        ...
        async getData() {
            try {
                this.isLoading = true;
                let url = `OrderProduct/filter?timeStart=${this.filter.timeStart}&timeEnd=${this.filter.timeEnd}&keyword=${this.keyword}&sort=${this.sort}&deliveryMethod=${this.filter.deliveryMethod}&paymentMethod=${this.filter.paymentMethod}&statusPayment=${this.filter.statusPayment}&status=${this.filter.statusOrder}&pageSize=${this.pageSize}&pageNumber=${this.pageNumber}`;
                await axios.get(url)
        ...
        */
        ...
        setPaymentStatusFilter(value) {
            Const.PaymentStatusFilter.find((item) => {
                if (item.Label == value) {
                    this.filter.statusPayment = item.Value;
                }
            });
        },
        ...
        setStatusOrder(value) {
            Const.StatusOrderFilter.find((item) => {
                if (item.Label == value) {
                    this.filter.statusOrder = item.Value;
                }
            });
        },
        ...
        deleteSingle(order) {
            try {
                ...
                // this.nameOrderDelete = order.orderID.replace(/-/g, '').toUpperCase();
                this.nameOrderDelete = order.orderID.toUpperCase();
        ...
        /**
         * Gửi yêu cầu update trạng thái đơn hàng đến Server
         * Author: HAQUAN(28/10/2022)
         */
        async requestUpdateStatus(orderID, status) {
            try {
                let url = `OrderProduct/updateStatus${orderID}?status=${status}`
                await axios.put(url).then((response) => {
                    console.log(response);
                    this.getData();

                    // Hiển thị thông báo xóa thành công
                    this.showToast(Resource.Message.UpdateOrderSucces, Const.TypeToast.Success);
                    this.getData();
                })
