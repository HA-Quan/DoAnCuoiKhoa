<template>
                            ...
                            <div class="flex mr-10 block">
                                ...
                                <div class="flex combobox-form">
                                    <BaseCombobox tabindex="16" :hasIcon="true" :isReadonly="true"
                                        ...
                                        fieldName="Label" @getValue="setStatusOrder" :disabled="formMode == Enum.Mode.Edit" />
                                </div>
                            </div>
                ...
                <div class="modal-footer">
                    <div class="flex-row flex-end">
                        ...
                        <el-tooltip effect="dark" :content="Const.KeyStrokes.CtrlShiftS" placement="top">
                            <BaseButton tabindex="20" @click="save(true)" @keydown.enter="save(true)"
                                v-show="!checkForm() && orderModel.listOrderDetail.length > 0"
                        ...
                        <el-tooltip effect="dark" :content="Const.KeyStrokes.CtrlS" placement="top">
                            <BaseButton tabindex="19" @click="save(false)" @keydown.enter="save(false)"
                                v-show="checkForm() && orderModel.listOrderDetail.length > 0" style="padding: 0 16px;"
    ...
    <AddProduct :closeFormAddProduct="closeFormAddProduct" @setListProduct="setListProduct" v-if="isShowFormAddProduct" :isImport="false" />
...
export default {
    components: {
        BaseCombobox, BaseButton, BasePopup, InputString, BaseRadio, CartItem, AddProduct, BaseToastMessage
    },
    ...
    methods: {
       ...
       */
        ...
        /**
         * Mở form thêm sản phẩm vào đơn hàng 
         * Author: HAQUAN(29/08/2023) 
         */
        addProduct() {
        ...
        setListProduct(listProduct) {
            try {
                listProduct.forEach(
                    (e) => {
                        if (!this.orderModel.listOrderDetail.some((p) => p.productID == e.productID)) {
                            this.orderModel.listOrderDetail.push(e);
                        }
                        else {
                            this.orderModel.listOrderDetail.find(
                                (el) => {
                                    if (el.productID == e.productID) {
                                        el.amount += e.amount;
                                    }
                                }
                            )
        ...
        async sendRequestUpdate() {
            try {
                this.orderModel.order.modifiedBy = this.$store.getters.user.accountID;
            ...
            try {
                ...
                await axios.get(url)
                    .then((response) => {
                        ...
                        console.log(this.orderModel.listOrderDetail);
                    })
