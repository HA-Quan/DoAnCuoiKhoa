                ...
                <div class="w-full flex-column">
                    ...
                    <div class="row-form flex">
                        ...
                        <div class="flex text-field-form">
                            <InputNumber :transmissionNumber="importModel.listImportDetail[0].quantity" :decimalPlaces="0" tabindex="2"
                                :hasIcon="true" :hasError="errors.amount != ''"
                                @eventBlur="focusAmount = false; validateAmount();" :isRef="focusAmount"
                                :nameProperty="Resource.ImportProductProperty.Quantity" @update="updateValue" />
                            <div class="error-text">{{ errors.amount }}</div>
                        </div>
                    </div>
                    <div class="row-form flex">
                        <label for="" class="label-form d-flex">
                            {{ Resource.Label.PriceImport }}
                            <span class="required">*</span>
                        </label>

                        <div class="flex text-field-form">
                            <InputNumber :transmissionNumber="importModel.listImportDetail[0].priceImport" :decimalPlaces="0" tabindex="3"
...
export default {
    ...
    methods: {
        ...
        updateValue(value, nameProperty) {
            this.importModel.listImportDetail[0][nameProperty] = value;
        },
        ...
        */
        importProduct() {
            try {
                ...
                if (valid) {
                    this.importModel.infoImport.price = this.importModel.listImportDetail[0].quantity * this.importModel.listImportDetail[0].priceImport;
                    // this.importModel.infoImport.productID = this.productId;
        ...
        */
        validateSupplier() {
            try {
                if (!this.importModel.infoImport.supplierID) {
                    this.errors.supplier= Resource.Error.Supplier;
                    return false;
                }   
                this.errors.supplier = '';
        ...
        */
        validateAmount() {
            try {
                if (!this.importModel.listImportDetail[0].quantity) {
        ...
        */
        validatePrice() {
            try {
                if (!this.importModel.listImportDetail[0].priceImport) {
    ...
    created() {
        ...
        this.importModel.listImportDetail.push(this.importDetail);
        this.importModel.listImportDetail[0].productID = this.productId;
        this.importModel.infoImport.createdBy = this.$store.getters.user.accountID;
        this.getData();
    }
}
